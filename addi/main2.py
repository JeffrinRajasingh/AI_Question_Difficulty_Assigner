...
class AIQuestionAssignmentSystem:
    ...
    def classify_question_difficulty(self, questions_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for i, question in enumerate(questions_df["Question"]):
                ...
                try:
                    # Enhanced prompt with better context for difficulty classification
                    prompt = f"""
                    Analyze this educational question and determine its difficulty level:
                    Question: "{question}"
                    ...
                    # Distribute fallbacks more evenly
                    fallback_difficulty = self.difficulty_labels[i % 3]  # Rotate through difficulties
                    predicted_difficulties.append(fallback_difficulty)
    ...
    def _balance_question_distribution(self, questions_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Identify levels that need more questions
        for difficulty in self.difficulty_labels:
            current_count = difficulty_counts.get(difficulty, 0)
            
            if current_count == 0:
                # No questions in this difficulty - need to reassign some
                logger.info(f"Rebalancing: {difficulty} level has 0 questions, reassigning...")
                
                # Find questions with lowest confidence from other levels
                other_questions = questions_df[questions_df["Question_Difficulty"] != difficulty].copy()
                other_questions = other_questions.sort_values("Confidence_Score", ascending=True)
                
                # Reassign lowest confidence questions to this level
                num_to_reassign = min(target_per_level, len(other_questions))
                reassign_indices = other_questions.head(num_to_reassign).index
                questions_df.loc[reassign_indices, "Question_Difficulty"] = difficulty
                
                logger.info(f"Reassigned {num_to_reassign} lowest-confidence questions to {difficulty} level")
                
            elif current_count < target_per_level:
                # Too few questions - try to get more
                needed = target_per_level - current_count
                
                # Find questions from over-represented levels with low confidence
                over_represented = questions_df[
                    (questions_df["Question_Difficulty"] != difficulty) & 
                    (questions_df["Confidence_Score"] < 0.6)
                ]
                
                # Prioritize taking from levels that have too many
                for other_difficulty in self.difficulty_labels:
                    if difficulty_counts.get(other_difficulty, 0) > target_per_level:
                        candidates = over_represented[
                            over_represented["Question_Difficulty"] == other_difficulty
                        ]
                        
                        if len(candidates) > 0:
                            to_move = min(needed, len(candidates), 
                                        difficulty_counts[other_difficulty] - target_per_level)
                            
                            if to_move > 0:
                                move_indices = candidates.head(to_move).index
                                questions_df.loc[move_indices, "Question_Difficulty"] = difficulty
                                logger.info(f"Moved {to_move} questions from {other_difficulty} to {difficulty}")
                                needed -= to_move
